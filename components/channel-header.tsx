...
import { CustomizeChannelModal } from "./customize-channel-modal" // Import new modal
import { useGame } from "@/components/game-provider"

interface ChannelHeaderProps {
  channel: PlayerChannel | BaseChannel // Can be player or other channel
  activeTab: "home" | "videos" | "playlists" | "about" | "posts" | "releases"
}

export function ChannelHeader({ channel, activeTab }: ChannelHeaderProps) {
  const { gameState } = useGame() // Use game state to check if it's the player's channel
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 px-4">
        ...
        <div className="flex-1 text-center sm:text-left">
          ...
          >
            <span className="text-sm font-medium">More about this Punster Profile</span> {/* Renamed text */}
          </Button>
          {!isPlayerChannel && (
          ...
          {isPlayerChannel && (
            <Button variant="ghost" size="icon" className="mt-2 sm:ml-2" onClick={() => setIsCustomizeModalOpen(true)}>
              ...
              <span className="sr-only">Customize Punster Profile</span> {/* Renamed text */}
            </Button>
          )}
        </div>
      </div>
      ...
      {isPlayerChannel && (
        <CustomizeChannelModal
          ...
          channel={channel as PlayerChannel} // Cast to PlayerChannel as it's the player's own channel
        />
      )}
    </div>
  )
